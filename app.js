...
let appPage, projectsListView, projectDetailView, dashboardContainer,
    dashboardKPIs, projectsContainer, addProjectButton, addFirstProjectButton,
...
// FUNGSI handleAuthSubmit, handleLogout, toggleAuthMode, showAuthPage DIHAPUS

const showAppPage = async () => {
    // appPage.classList.remove('hidden'); // app-page sekarang selalu terlihat
...
const fetchProjects = async () => {
    ...
    try {
        const { data, error } = await supabaseClient
            .from('projects')
            .select('*')
            // .eq('user_id', currentUser.id) DIHAPUS
        ...
        alert('Gagal memuat data proyek: ' + error.message + '\n\nPastikan Anda telah menjalankan skrip SQL untuk menonaktifkan RLS.');
        renderDashboard([]); 
        renderProjectCards([]); 
    }
};
...
const populateActivityTypeCheckboxes = () => {
    if (!projectActivityTypeCheckboxes) return;
...
const handleProjectFormSubmit = async (event) => {
    ...
    const projectData = {
        ...
        // user_id: currentUser.id DIHAPUS
    };
    ...
    if (response.error) {
        ...
        if (venueCategoryValue && !userVenueCategories.includes(venueCategoryValue)) {
            await supabaseClient.from('user_venue_categories').insert({ category_name: venueCategoryValue }); // user_id DIHAPUS
            await fetchUserVenueCategories();
        }
        if (venueNameValue && !userVenueNames.includes(venueNameValue)) {
            await supabaseClient.from('user_venue_names').insert({ venue_name: venueNameValue }); // user_id DIHAPUS
            await fetchUserVenueNames();
        }
        closeProjectModal();
        fetchProjects();
        // Blok notifikasi email DIHAPUS
    }
};
...
const handleCopyProject = async () => {
    ...
    try {
        ...
        // user_id (jika ada) akan ikut tersalin, tapi tidak masalah karena RLS nonaktif
...
const initializeApp = () => {
    ...
    // Hapus onAuthStateChange dan panggil showAppPage() secara langsung
    showAppPage();
};
