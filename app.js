...
const renderDashboard = (projects) => {
    ...
    dashboardKPIs.innerHTML = `
        <div class="bg-white p-4 rounded-lg shadow flex items-center">
            <div class="bg-indigo-100 rounded-full p-3 mr-4"><svg class="w-6 h-6 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg></div>
            <div><p class="text-sm text-gray-500">Total Proyek</p><p class="text-2xl font-bold">${projects.length}</p></div>
        </div>
        <div class="bg-white p-4 rounded-lg shadow flex items-center">
            <div class="bg-green-100 rounded-full p-3 mr-4"><svg class="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v.01"></path></svg></div>
            <div><p class="text-sm text-gray-500">Total Budget</p><p class="text-2xl font-bold">${formatCurrency(totalBudget)}</p></div>
        </div>
        <div class="bg-white p-4 rounded-lg shadow flex items-center">
            <div class="bg-yellow-100 rounded-full p-3 mr-4"><svg class="w-6 h-6 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg></div>
            <div><p class="text-sm text-gray-500">Sedang Berjalan</p><p class="text-2xl font-bold">${inProgress}</p></div>
        </div>
        <div class="bg-white p-4 rounded-lg shadow flex items-center">
            <div class="bg-red-100 rounded-full p-3 mr-4"><svg class="w-6 h-6 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg></div>
            <div><p class="text-sm text-gray-500">Mendekati Deadline</p><p class="text-2xl font-bold">${nearingDeadline}</p></div>
        </div>
    `;
    ...
    // 3. Render Deadline List
    const sortedByDeadline = projects
    ...
    if (sortedByDeadline.length === 0) {
        ...
        sortedByDeadline.slice(0, 5).forEach(project => { // Tampilkan 5 teratas
            ...
            deadlineEl.innerHTML = `
                ...
                <p class="text-sm text-gray-600">${formatDate(project.end_date)} (${remainingDays} hari lagi)</p>
            `;
        ...
        tasks.forEach(task => {
            ...
            taskEl.innerHTML = `
                ...
                <div class="flex items-center gap-2">
                    <span class="text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full text-white ${statusColor}">
                        ${task.status}
                    </span>
...
const handleProjectFormSubmit = async (event) => {
    ...
    if (isUpdate) {
        // Saat update, kita tidak mengubah budget dari form ini.
    } else {
...
const handleExportToExcel = async () => {
    ...
    const projectSummary = [
        ...
        { A: "Budget Awal", B: formatCurrency(currentProject.initial_budget) },
        { A: "Budget Disesuaikan", B: formatCurrency(currentBudget) },
        { A: "Total Biaya Tugas", B: formatCurrency(totalCost) },
        { A: "Sisa Budget", B: formatCurrency(currentBudget - totalCost) },
